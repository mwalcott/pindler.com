...
//	print_r($atts);
	...
	if(isset($options['settings'])){
		foreach($options['settings'] as $k => $v)
		{
			switch($k)
			{
				...
				break;
				case 'dates':
				$v = $v == '' ? 'min,mins,hr,hrs,day,days,week,weeks,month,months,year,years' : $v ;
				if(isset($atts[$k])){
					$opt .= ',dates: "'.$atts[$k].'"' ;
				} else {
					$opt .= $v != $settings[$k] ? ',dates: "'.$v.'"' : '' ;
				}
				break;
				default:
				if(isset($atts[$k])){
	...
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.5.18.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
...
//	print_r($stream);
	...
	$out .='<script type="text/javascript">jQuery(document).ready(function($){';
	$out .= 'var config = '.$config.';';
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.wall.1.6.js", function(){}); $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.5.18.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
