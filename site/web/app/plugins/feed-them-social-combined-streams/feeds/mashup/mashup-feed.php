...
class FTS_Mashup_Feed extends feed_them_social_functions
{
    ...
    function fts_mashup_head()
    {
        ...
        wp_enqueue_script('fts-images-loaded', plugins_url('feed-them-social/feeds/js/imagesloaded.pkgd.min.js'), array('jquery'));
    }
    /**
    ...
    function fts_dateSort($a, $b)
    {
        ...
        if (isset($a->created_time)) {
            ...
            } // faceboook timestamp looks like 2017-03-29T22:01:37+0000
            else {
                $dateA = strtotime($a->created_time);
            }
        }
        // twitter and pinterest timestamp
        // twitter timestamp looks like Wed Mar 29 22:40:59 +0000 2017
        // pinterest timestamp looks like 2016-07-28T17:22:31
        if (isset($a->created_at)) {
            $dateA = strtotime($a->created_at);
        }
        // youtube timestap
        // youtube timestamp looks like 2017-03-29T20:00:00.000Z
        if (isset($a->snippet->publishedAt)) {
            $dateA = strtotime($a->snippet->publishedAt);
        }
        // wp gallery timestap
        // wp gallery timestamp looks like 2017-03-25 12:57:18
        if (isset($a->post_date)) {
            $dateA = strtotime($a->post_date_gmt);
        }
        ////////////////////////////////////
        /////////////// dateB /////////////
        ///////////////////////////////////
        ...
        if (isset($b->post_date)) {
            $dateB = strtotime('2017-04-07 21:54:04');
        }
    ...
    function wp_gallery_post_images($fts_wp_gallery_post_id, $social_network_posts) {
        //19911
        $wp_gallery_feed['data'] = get_attached_media( 'image', $fts_wp_gallery_post_id );

        $wp_gallery_feed = (object)$wp_gallery_feed;

        foreach ($wp_gallery_feed->data as $post_array) {
                $post_array->fts_feed_type = 'wp_gallery';
        }

        $wp_gallery_feed = array_reverse($wp_gallery_feed->data);
        $wp_gallery_feed = array_slice($wp_gallery_feed, 0, $social_network_posts);

      //  echo '<pre>';
      //  print_r($wp_gallery_feed);
      //  echo '</pre>';


      //  }
        return $wp_gallery_feed;
    }
    /**
     * FTS Mashup Function
     *
    ...
    function fts_mashup_func($atts)
    {
       ...
       // $fts_instagram_username = $instagram_name; // 28902942 id is for gopro
        $fts_wp_gallery_name = $wp_gallery_name;
        $fts_instagram_username = $instagram_name;
        $fts_instagram_location = $instagram_location_id;
        ...
        if (false !== ($transient_exists = $mashup_functions->fts_check_feed_cache_exists($data_cache))) {
            ...
            if (!empty($fts_wp_gallery_name)) {
                // WP GALLERY

                //Display Images Gallery
               // $display_gallery = new Display_Gallery();
               // $image_list = $display_gallery->get_media_rest($this->parent_post_id, '100');

                $wp_gallery_feed = $this->wp_gallery_post_images($fts_wp_gallery_name, $social_network_posts);
                ...
                $wp_gallery_feed = array();
            }

            if (!empty($instagram_name) || !empty($instagram_type)) {
                // INSTAGRAM
                //URL to get Feeds
                if ($instagram_type == 'hashtag') {
                    $instagram_data_array['data'] = 'https://api.instagram.com/v1/tags/' . $fts_instagram_username . '/media/recent/?count=' . $social_network_posts . '&access_token=' . $fts_instagram_access_token;
                }
                elseif($instagram_type == 'location') {
                    $instagram_data_array['data'] = 'https://api.instagram.com/v1/locations/' . $fts_instagram_location . '/media/recent/?count=' . $social_network_posts . '&access_token=' . $fts_instagram_access_token;
                }else {
            ...
            if (!empty($twitter_name) || !empty($search)) {
                // TWITTER
                ...
                if (isset($error_check)) {
                    ...
                    $limitToDisplay = min($social_network_posts, count($fetchedTweets));
                    for ($i = 0; $i < $limitToDisplay; $i++) {
                        $social_network_posts = $limitToDisplay;
                        break;
                    }
                  //    echo'<pre>';
                  //     print_r($limitToDisplay);
                  //    echo'</pre>';
            ...
            $fts_list_arrays = array_merge($wp_gallery_feed, $facebook_feed, $instagram_feed, $pinterest_feed, $youtube_feed, $fetchedTweets);
          //  $fts_list_arrays = array_merge($facebook_feed, $wp_gallery_feed);
             // var_dump( $fts_list_arrays);

          //  print '<pre>';
          //  print_r($fts_list_arrays);
          //  print '</pre>';
      ...
      //   print '</pre>';
        if (isset($grid) && $grid == 'yes') {
             $output = '<script>jQuery(window).load(function () {setTimeout(function() {jQuery(".masonry").masonry("layout");}, 200);});</script>';
           // $output .= 'jQuery(".masonry").imagesLoaded().progress( function() { ';
        ...
        $fts_version_check = ftsystem_version();
        ...
        foreach ($feed_data->data as $post_data) {
            if ($count <= $posts) {
                ...
                Switch ($fts_feed_type) {
                    ...
                    case 'instagram':
                        ...
                        if (!isset($post_data->error_message)) {
                            ...
                            if ($instagram_type == 'hashtag' || $instagram_type == 'location') {
                                $instagram_user = $post_data->user->username ? $post_data->user->username : "";
                                $full_name = $post_data->user->full_name ? $post_data->user->full_name : $post_data->user->username;
                            }
                           ...
                           // $created_time = $post_data->post_date;
                    ...
                    case 'youtube':
                        if (!isset($post_data->error_message)) {
                            ...
                            if(isset($post_data->snippet->resourceId->videoId)){
                                // user or playlist video id
                            $youtube_video_id = $post_data->snippet->resourceId->videoId;
                            }
                            elseif (isset($post_data->id->videoId)){
                                // channel id
                                $youtube_video_id = $post_data->id->videoId;
                            }
                            else{
                                $youtube_video_id = '';
                            }
                    ...
                    case 'wp_gallery':
                        ...
                        $user_info = get_user_by('id', $post_data->post_author);

                        $user_name_href = get_option('home');

                        if($wp_gallery_display_name == 'username') {
                            $user_name = $user_info->user_login;
                        }
                        elseif($wp_gallery_display_name == 'first_name') {
                            $user_name = $user_info->first_name;
                        }
                        elseif ($wp_gallery_display_name == 'first_last_name'){
                            $user_name =  $first_name = $user_info->first_name .' '.$user_info->last_name;
                        }

                        //Created Time
                        $created_time = $post_data->post_date;

                        $attachment_meta = $this->fts_wp_get_attachment($post_data->ID);

                        $wp_gallery_description = '<p>'.$attachment_meta['description'].'</p>';
                        //Image Link
                        $wp_gallery_image_link = $attachment_meta['src'];
                        //Post Link
                        $wp_gallery_post_link = $attachment_meta['href'];


                        $wp_gallery_posts_media = '<a href="' . $wp_gallery_post_link . '" target="_blank"><img class="fts-mashup-instagram-photo" src="' . $wp_gallery_image_link . '"/></a>';

                        $wp_gallery_posts_view_photo_text = __('View Photo', 'feed-them-social');


                        // } else {
                        //  $instagram_description = 'error';
                        // }
                        break;
                }
                ...
                if(isset($words) && !empty($words)){
                    ...
                    $wp_gallery_description = $this->fts_custom_trim_words(isset($wp_gallery_description) ? $wp_gallery_description : '', $words, $more);
                }
                ...
                $set_zero = 0;
                // we are not using single event posts in mashup soo variable is empty
                ...
                if ($post_data->fts_feed_type !== 'facebook') {
                    ...
                    if ($fts_feed_type == 'wp_gallery') {
                        //WP Gallery description
                        $output .= $wp_gallery_description;
                    }
                    ...
                    if ($fts_feed_type == 'wp_gallery' && !isset($post_data->error_message)) {
                        //Instagram Media
                        $output .= $wp_gallery_posts_media;
                    }
                    ...
                    // This function fts_share_option option was added the fts free functions.
                    // No fatal error with this check in case combined user updated this plugin version 1.0.4 before update fts free from 2.1.9 to 2.2.0
                    if($fts_version_check >= '2.2.0') {
                        $share_this = new feed_them_social_functions();
                        $fts_share_option_twitter = $share_this->fts_share_option(isset($permalink) ? $permalink : NULL, $posts_description);
                        $fts_share_option_instagram = $share_this->fts_share_option(isset($instagram_post_link) ? $instagram_post_link : NULL, $instagram_description );
                        $fts_share_option_pinterest = $share_this->fts_share_option(isset($pinterest_link) ? $pinterest_link : NULL, $pinterest_description);
                        $fts_share_option_youtube = $share_this->fts_share_option(isset($youtube_video_url) ? $youtube_video_url : NULL, $youtube_description);
                        $fts_share_option_wp_gallery = $share_this->fts_share_option(isset($wp_gallery_post_link) ? $wp_gallery_post_link : NULL, isset($wp_gallery_posts_media) ? $wp_gallery_posts_media : NULL);
                    }
                    else {
                        $fts_share_option_twitter = '';
                        $fts_share_option_instagram = '';
                        $fts_share_option_pinterest = '';
                        $fts_share_option_youtube = '';
                        $fts_share_option_wp_gallery = '';
                    }


                    $output .= '<div class="fts-mashup-count-wrap">';
                    if ($fts_feed_type == 'twitter' && !isset($post_data->error_message)) {
                        $fts_twitter_full_width = get_option('twitter_full_width')  == 'yes' ? 'fts-twitter-full-width' : 'fts-twitter-no-margin-left';
                        $output .= $fts_share_option_twitter;
                        $output .= '<div class="fts-twitter-reply-wrap '.$fts_twitter_full_width.'">';
                        $output .= '</div>';

                        //Twitter Favorite Icon and Count
                        $output .= '<div class="fts-twitter-reply-wrap-left">';
                    ...
                    if ($fts_feed_type == 'instagram' && !isset($post_data->error_message)) {
                        ...
                        $output .= $fts_share_option_instagram;
                        $output .= '<div class="fts-instagram-reply-wrap-left">';
                    ...
                    if ($fts_feed_type == 'pinterest' && !isset($post_data->error_message)) {
                        ...
                        $output .= $fts_share_option_pinterest;
                        $output .= '<div class="fts-pinterest-reply-wrap-left">';
                    ...
                    if ($fts_feed_type == 'youtube' && !isset($post_data->error_message)) {
                        ...
                        $output .= '<div class="fts-youtube-reply-wrap-left">';
                        $output .= '<a href="' . $youtube_video_url . '" target="_blank">';
                        $output .= __('View on YouTube', 'feed-them-social');;
                        $output .= '</a>';
                    ...
                    if ($fts_feed_type == 'wp_gallery' && !isset($post_data->error_message)) {
                        $output .= $fts_share_option_wp_gallery;
                        //Wp Gallery Favorite Icon and Count
                        $output .= '<a href="' . $wp_gallery_post_link . '" target="_blank">';
                        $output .= $wp_gallery_posts_view_photo_text;
                        $output .= '</a>';
                        $output .= '</div><div class="fts-clear"></div>';
                    }
    ...
    function fts_wp_get_attachment( $attachment_id ) {
